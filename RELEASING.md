...
`CONTRIBUTING.md` tells people to email the maintainer about vulnerabilities
but gives no address. Add `SECURITY.md` with a real contact before the repo is
public, and use an address chosen for the purpose rather than a personal one —
the package metadata deliberately carries no email.
